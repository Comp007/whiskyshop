  ...
  Future<void> soumettreNote() async {
    if (selectedGerantUid == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Veuillez choisir un gérant.")),
      );
      return;
    }

    final firestore = FirebaseFirestore.instance;
    final gerantRef = firestore.collection('users').doc(selectedGerantUid);
    final currentUser = FirebaseAuth.instance.currentUser;
    ...
    try {
      ...
      // 1) Mise à jour de la note globale du gérant
      await gerantRef.update({
        'note': double.parse(nouvelleNote.toStringAsFixed(1)),
        'totalNotes': totalNotes + 1,
      });

      // 2) Enregistrement dans la collection notes
      await firestore.collection('notes').add({
        'gerantId': selectedGerantUid,
