...
class GestionPointVente extends StatefulWidget {
  ...
  @override
  State<GestionPointVente> createState() => _GestionPointVenteState();
}
          ...
          content: SingleChildScrollView(
            child: Column(
              children: [
                ...
                ...employes.map((e) {
                  final estDejaAffecte = e['pointDeVenteId'] != null &&
                      e['pointDeVenteId'].toString().isNotEmpty;


                  return CheckboxListTile(
                    title: Text(
                      "${e['fullName']}${estDejaAffecte ? ' (déjà affecté)' : ''}",
                      style: estDejaAffecte ? TextStyle(color: Colors.grey) : null,
                    ),
                    value: employesSelectionnes.contains(e['id']),
                    onChanged: estDejaAffecte
                        ? null // désactive la case
                        : (bool? selected) {
                      setState(() {
                        if (selected == true) {
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final docRef = await _firestore.collection('points_vente').add({
                ...
                for (String employeId in employesSelectionnes) {
                  await _firestore.collection('users').doc(employeId).update({
                    'pointDeVenteId': docRef.id,
                  });
                }
            ...
            child: Column(
              children: [
                ...
                ...employes.map((e) {
                  final estDejaAffecte = e['pointDeVenteId'] != null &&
                      e['pointDeVenteId'].toString().isNotEmpty &&
                      e['pointDeVenteId'] != pointVente['id'];
                  return CheckboxListTile(
                    title: Text(
                      "${e['fullName']}${estDejaAffecte ? ' (déjà affecté)' : ''}",
                      style: estDejaAffecte ? TextStyle(color: Colors.grey) : null,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                List<String> anciensEmployes = List<String>.from(pointVente['employes'] ?? []);

                // Pour chaque employé retiré, on enlève le champ pointVenteId
                for (String ancienEmployeId in anciensEmployes) {
                  if (!employesSelectionnes.contains(ancienEmployeId)) {
                    await _firestore.collection('users').doc(ancienEmployeId).update({
                      'pointDeVenteId': FieldValue.delete(),
                    });
                  }
                }

                // Pour chaque nouvel employé ajouté (qui n’était pas dans anciensEmployes), on met à jour pointVenteId
                for (String employeId in employesSelectionnes) {
                  if (!anciensEmployes.contains(employeId)) {
                    await _firestore.collection('users').doc(employeId).update({
                      'pointDeVenteId': pointVente['id'],
                    });
                  }
                }
